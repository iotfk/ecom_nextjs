...
export async function PUT(request) {
    ...
    const deleteType = payload.deleteType || 'SD'
        ...
        const productVariant = await ProductVariantModel.find({ _id: { $in: ids } })
...
if (!['SD', 'RSD'].includes(deleteType)) {
    return response(false, 400, 'Invalid delete type');
        ...
        if (deleteType === 'SD') {
            ...
            result = await ProductVariantModel.updateMany(
                { _id: { $in: ids } },
...
export async function DELETE(request) {
    try {
        ...
        return response(
            ...
            200, 
            `${result.deletedCount} 
            ${result.deletedCount === 1 ? 'product variant' : 'product variants'} permanently deleted`
        );
