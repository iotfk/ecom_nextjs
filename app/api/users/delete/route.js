...
export async function PUT(request) {
    ...
    const deleteType = payload.deleteType || 'SD'
        ...
        if (!users || users.length === 0) {
            return response(false, 400, 'No user with provided ID');
        }
...
if (!['SD', 'RSD'].includes(deleteType)) {
    return response(false, 400, 'Invalid delete type');
        ...
        return response(
            true, 
            200, 
            `${result.modifiedCount} ${result.modifiedCount === 1 ? 'user' : 'users'}
...
export async function DELETE(request) {
    try {
        ...
        return response(
            ...
            200, 
            `${result.deletedCount} 
            ${result.deletedCount === 1 ? 'user' : 'users'} permanently deleted`
                );
