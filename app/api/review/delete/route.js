...
export async function PUT(request) {
    ...
    const deleteType = payload.deleteType || 'SD'
        ...
        if (!reviews || reviews.length === 0) {
            return response(false, 400, 'No review with provided ID');
        }
...
if (!['SD', 'RSD'].includes(deleteType)) {
    return response(false, 400, 'Invalid delete type');
        ...
        if (deleteType === 'SD') {
            ...
            result = await ReviewModel.updateMany(
                { _id: { $in: ids } },
        ...
        return response(
            true, 
            200, 
            `${result.modifiedCount} ${result.modifiedCount === 1 ? 'review' : 'reviews'}
...
export async function DELETE(request) {
    try {
        ...
        return response(
            ...
            200, 
            `${result.deletedCount} 
            ${result.deletedCount === 1 ? 'review' : 'reviews'} permanently deleted`
                );
